...
- **New theme family: Rebecca.** Built around `#663399`, Rebecca Purple, the CSS color named in
  memory of Rebecca Meyer. Every purple is that color's own hue, and all four heading levels are
  purple, with orchid, periwinkle and a leaf green beside it. It comes in dark and light, each with a
  **No Background** twin: `rebecca-dark`, `rebecca-dark-no-background`, `rebecca-light`,
  `rebecca-light-no-background`. All four pass WCAG 2.2 AA.
- **New backdrop: flowers.** Small violets, forget-me-nots and daisies, with buds and leaves, drift
  slowly down behind Rebecca in Rebecca Purple. Like NEO's rain, the motion toggle and
  `prefers-reduced-motion` stop it, and its brightest point is checked as a surface text sits on.
- **Two new backdrop tokens.** Every theme now also sets `--fx-backdrop-mask-size` and
  `--fx-backdrop-timing`, so each pattern carries its own tile size and pace. Existing themes look
  exactly as before.

### Upgrading

1. **Vendor `theme.css` and `effects.css` together.** The flower tiles live in `effects.css`. A new
   `theme.css` over an older `effects.css` shows Rebecca with no backdrop (never a solid wash), and
   an older `theme.css` over the new `effects.css` still rains as before.
2. **Apps with the generated selector (`theme-select.js`):** Rebecca shows up in the list on its
   own. No token was renamed or removed.
3. **Apps that offer Rebecca:** include the "Reduce motion" control (`data-motion-toggle`), as for
   NEO; the flowers drift until motion is off (WCAG 2.2.2). A CSP that restricts `img-src` must allow
   `data:` for the flowers to be drawn.
4. **Apps with a hand-built theme list:** add the four `rebecca-*` ids if you want Rebecca.

---

**For theme authors.** `backdrop: 'flowers'` joins `'grid'` and `'rain'`, painted in the new
`backdropColor` (default: the purple accent; rain keeps `rainColor` too). The tiles are generated by
`tools/flowers.mjs`. The draft-5 review page also gets a collapsible header capped at 80% of the
viewport, so small screens can always see the palettes.
